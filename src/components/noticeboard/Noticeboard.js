...
class Noticeboard extends React.PureComponent {
  ...
  render() {
    ...
    const unslicedNotices = this.props.notices.filter((notice) => {
    ...
    return (
      <div className={classes.marginTopStandard}>
        ...
        {this.state.noticeLimit < unslicedNotices.length && (
          <div className={classes.flexRowCentered}>
            <Button
              onClick={() => {
                this.setState({ noticeLimit: this.state.noticeLimit + 12 })
              }}
            >
