...
class AsbestosSampleEditModal extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (
      this.state.activeSample !== nextState.activeSample ||
  ...
  loadProps = (activeSample) => {
    ...
    if (waAnalysis) {
      ...
      if (samples[activeSample].waLayerNum === undefined)
        samples[activeSample].waLayerNum = { lt2: waLayerNum, to7: waLayerNum, gt7: waLayerNum }
  ...
  saveSample = async (activeSample) => {
    ...
    if (this.props.samples[this.props.modalProps.activeCoc][this.state.activeSample] && (resultChanged || weightChanged)) {
      recordAnalysis(
        batch,
        this.props.analyst,
        sample,
  ...
  saveMultipleSamples = async () => {
    ...
    await Object.values(this.state.samples).forEach((sample) => {
      ...
      if (originalSamples[sample.sampleNumber] && (resultChanged || weightChanged)) {
        recordAnalysis(
          batch,
          this.props.analyst,
          sample,
            ...
            <DialogContent>
              ...
              <Grid container alignItems='flex-start' justify='flex-end'>
                ...
                <Grid item xs={6}>
                  ...
                  <div className={classes.flexRow}>
                    <div className={classes.formInputMediumRequired}>
                      <TextyBox
                        ...
                        helperText={'REQUIRED Record the weight as received (tape samples can be recorded as <0.1).'}
                        end={'g'}
                        numericOnly={true}
                        dp={1}
                      />
                    ...
                    <div className={classes.formInputMedium}>
                      <TextyBox
                        ...
                        label={'Weight of Subsample'}
                        helperText={'Record the weight of the subsample if the entire sample is not analysed.'}
                        end={'g'}
                        numericOnly={true}
                        dp={1}
                      />
                    ...
                    <div className={classes.formInputMedium}>
                      <TextyBox
                        ...
                        field={'weightDry'}
                        label={'Dry Weight'}
                        helperText={'Record the weight after drying (~105°).'}
                        end={'g'}
                        numericOnly={true}
                        dp={1}
                      />
                    ...
                    <div className={classes.formInputMedium}>
                      <TextyBox
                        ...
                        field={'weightAshed'}
                        label={'Ashed Weight'}
                        helperText={'Record the weight after ashing (~400°).'}
                        end={'g'}
                        numericOnly={true}
                        dp={1}
                      />
                  ...
                  {sampleMoisture && <div className={classes.informationBoxRounded}>Moisture: {sampleMoisture}%</div>}
                  {sample.material !== 'soil' && (
                    <div>
                      ...
                      <div className={classes.flexRow}>
                        ...
                        {sampleDimensions && <span className={classes.informationBoxRounded}>{sampleDimensions}</span>}
                      </div>
                    </div>
                  )}
                  ...
                  <TextyBox
                    ...
                    field={'labComments'}
                    helperText={
                      'Note description of the material or any additional observations or comments. This will not be displayed on the certificate.'
                    }
                    multiline={true}
                  />
                  <TextyBox
                    ...
                    field={'footnote'}
                    helperText={
                      'Add a footnote to be included in the issued report. This will be displayed as a footnote on the certificate.'
                    }
                    multiline={true}
                  />
                  {sample.material === 'soil' && (
              ...
              <div className={classes.flexRowLeftAlignEllipsis}>
                <div className={classes.subHeading}>Reported Asbestos Result</div>
              </div>
              ...
              {sample.confirm && (
                <div>
                  ...
                  {Object.keys(sample.confirm).map((key) => {
                    if (key !== 'totalNum' && sample.confirm[key].deleted !== true)
                      return <AsbestosSampleEditConfirmRow key={sample.confirm[key].date} confirm={sample.confirm[key]} that={this} />
                  })}
                </div>
              )}
              {waAnalysis && (
                <div>
                  ...
                  <AsbestosSampleWASummary
                    ...
                    acmInSoilLimit={
                      this.props.cocs[modalProps.activeCoc] && this.props.cocs[modalProps.activeCoc].acmInSoilLimit
                        ? parseFloat(this.props.cocs[modalProps.activeCoc].acmInSoilLimit)
                        : 0.01
                    }
                  />
...
export default withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(AsbestosSampleEditModal))
