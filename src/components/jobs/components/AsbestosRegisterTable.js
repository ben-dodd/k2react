...
=======
        ...
        columns={[
          ...
          },
          {
            ...
            accessor: (d) => d.materialRisk && d.materialRisk.score,
            Cell: (c) =>
              c.original.materialRisk && c.original.materialRisk.score ? (
                <span
                  className={classes[`colorsCell${c.original.materialRisk.color}`]}
                >{`${c.original.materialRisk.text} (${c.original.materialRisk.score})`}</span>
              ) : (
                <span />
              ),
            maxWidth: 120
          },
          {
            ...
            accessor: (d) => d.priorityRisk && d.priorityRisk.score,
            Cell: (c) =>
              c.original.priorityRisk && c.original.priorityRisk.score ? (
                <span
                  className={classes[`colorsCell${c.original.priorityRisk.color}`]}
                >{`${c.original.priorityRisk.text} (${c.original.priorityRisk.score})`}</span>
              ) : (
                <span />
              ),
            maxWidth: 120
          },
          {
            ...
            accessor: (d) => d.totalRisk && d.totalRisk.score,
            Cell: (c) =>
              c.original.totalRisk && c.original.totalRisk.score ? (
                <span
                  className={classes[`colorsCell${c.original.totalRisk.color}`]}
                >{`${c.original.totalRisk.text} (${c.original.totalRisk.score})`}</span>
              ) : (
                <span />
              ),
            maxWidth: 120
