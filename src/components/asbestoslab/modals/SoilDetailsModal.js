...
=======
                  ...
                  <FormHelperText style={{ width: 500 }}>
                    Select the major category of the soil. If over 35% clay or silt then it is fine. If there is enough organic material in
                    the sample to make it behave differently to clay or silt, then it is organic.
                  </FormHelperText>
                </FormControl>
                <div style={{ display: 'flex', flexDirection: 'row', marginTop: 16 }}>
                  <div>
...
=======
                ...
                <FormControl component='fieldset' margin='dense' style={{ marginBottom: 4 }}>
                  <FormGroup row>
                    {[
                      { label: 'Cobbles', name: 'cobbles' },
                      { label: 'Gravel', name: 'gravel' },
                      { label: 'Sand', name: 'sand' },
                      { label: 'Silt', name: 'silt' },
                      { label: 'Clay', name: 'clay' },
                      { label: 'Topsoil', name: 'organic topsoil' },
                      { label: 'Organic Sand', name: 'organic sand' },
                      { label: 'Organic Silt', name: 'organic silt' },
                      { label: 'Organic Clay', name: 'organic clay' },
                      { label: 'Peat', name: 'peat' }
                    ].map((check) => {
                      return (
                        <FormControlLabel
                          key={check.name}
                          control={
                            <Checkbox
                              checked={
                                sample.someFractionTypes && sample.someFractionTypes[check.name]
                                  ? sample.someFractionTypes[check.name]
                                  : false
                              }
                              onChange={() => this.handleCheck('someFractionTypes', check.name)}
                              value={check.name}
                            />
                          }
                      ...
                      return (
                        <FormControlLabel
                          ...
                          control={
                            <Checkbox
                              checked={
                                sample.minorFractionTypes && sample.minorFractionTypes[check.name]
                                  ? sample.minorFractionTypes[check.name]
                                  : false
                              }
                              onChange={() => this.handleCheck('minorFractionTypes', check.name)}
                              value={check.name}
                            />
                          }
                          disabled={sample.majorFractionType === check.name || sample.subFractionType === check.name}
                      ...
                      return (
                        <FormControlLabel
                          ...
                          control={
                            <Checkbox
                              checked={
                                sample.traceFractionTypes && sample.traceFractionTypes[check.name]
                                  ? sample.traceFractionTypes[check.name]
                                  : false
                              }
                              onChange={() => this.handleCheck('traceFractionTypes', check.name)}
                              value={check.name}
                            />
                          }
                          disabled={sample.majorFractionType === check.name || sample.subFractionType === check.name}
              ...
              <hr />
              <div>
                <div className={classes.heading} style={{ marginTop: 12 }}>
                  ...
                  <div>
                    <FormControl>
                      ...
                      >
                        ...
                        <option value={'gap graded'}>
                          Gap graded (absence of one or more intermediate sizes within what otherwise would be a well graded material)
                        </option>
                      </Select>
                    </FormControl>
...
=======
                  ...
                  >
                    <option value='' />
                    <option value={'loosely packed'}>
                      Loosely packed (can be removed from exposures by hand or removed easily by shovel)
                    </option>
                    <option value={'tightly packed'}>
                      Tightly packed (requires a pick for removal, either as lumps or as disaggregated material)
                    </option>
                  </Select>
                </FormControl>
              )}
              {sample.type === 'fine' && (
                <div>
                  ...
                  {sample.strength && sample.sensitivityStrength && (
                    <div style={{ marginTop: 12 }}>
                      <InputLabel>Sensitivity</InputLabel>
                      {getSoilSensitivity(sample)}
                    </div>
                  )}
                </div>
              )}
...
=======
                  ...
                  >
                    ...
                    <option value={'blocky'}>
                      Blocky (cohesive soil that can be broken down into angular lumps that resist further breakdown)
                    </option>
            ...
            <DialogActions>
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  let log = {
                    type: 'Analysis',
                    log: `Sample ${this.props.modalProps.doc.sampleNumber} (${this.props.modalProps.doc.description} ${this.props.modalProps.doc.material}) geotechnical information edited.`,
                    sample: this.props.modalProps.doc.uid,
                    chainOfCustody: this.props.modalProps.doc.cocUid
                  }
                  modalProps.onExit(this.state.sample)
...
export default withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(SoilDetailsModal))
>>>>>>> 19df57755d0c04c09358c8f67c601c2eec2f6e8d
