...
import { dateOf, personnelConvert, sentenceCase, titleCase } from 'actions/helpers'
  ...
  return listType === 'heading' ? (
    <div>
      <div className={classNames(classes.flexRow, classes.headingInline)}>
        ...
        <div className={classes.columnMedSmall}>Building or General Area</div>
        <div className={classes.columnMedSmall}>Room or Specific Area</div>
        <div className={classes.columnMedLarge}>Description</div>
        <div className={classes.columnMedLarge}>Material</div>
        <div className={classes.columnMedSmall}>Sample Category</div>
        {!doc.historicalCoc && <div className={classes.columnMedLarge}>Sampled By</div>}
        {!doc.historicalCoc && <div className={classes.columnMedSmall}>Sample Date</div>}
        {doc.historicalCoc && <div className={classes.columnExtraLarge}>Result</div>}
        <div className={classes.columnSmall} />
      </div>
    ...
    <div className={classes.flexRowHover}>
      <div className={classes.spacerSmall} />
      <div className={classes.columnSmall}>
        <div className={classes.circleShaded}>{i + 1}</div>
      </div>
      ...
      {!doc.historicalCoc && (
        <div className={classes.columnMedSmall}>
          <DatePicker
            value={sample.sampleDate !== undefined ? sample.sampleDate : that.state.defaultSampleDate}
            autoOk
      ...
      {doc.historicalCoc && (
        <div className={classNames(classes.flexRow, classes.columnExtraLarge)}>
          {['ch', 'am', 'cr', 'umf', 'no'].map((res) => {
            return AsbButton(that.props.classes[`colorsButton${colors[res]}`], that.props.classes[`colorsDiv${colors[res]}`], res, () =>
              that.handleResultClick(res, i)
            )
          })}
        </div>
      )}
    ...
    <div className={disabled ? classes.flexRowHoverDisabled : classes.flexRowHover} key={i}>
      ...
      <div className={classNames(classes.paddingSidesSmall)}>{writeDescription(sample)} (Bulk Sample)</div>
    </div>
  ) : (
    <div className={disabled ? classes.flexRowHoverDisabled : classes.flexRowHover} key={i}>
      <div className={classes.spacerSmall} />
      ...
      <div className={classNames(classes.paddingSidesSmall, classes.columnMedLarge)}>{sample.description ? sample.description : ''}</div>
      <div className={classNames(classes.paddingSidesSmall, classes.columnMedLarge)}>{sample.material ? sample.material : ''}</div>
      <div className={classes.columnMedSmall}>{sample.category ? sample.category : ''}</div>
      {!doc.historicalCoc && (
        <div className={classes.columnMedLarge}>{sample.sampledBy ? sample.sampledBy.map((e) => e.name).join(', ') : ''}</div>
      )}
      {!doc.historicalCoc && (
        <div className={classes.columnMedSmall}>
