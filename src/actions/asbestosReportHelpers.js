    ...
    Object.values(site.layout).forEach((roomGroup) => {
      ...
      roomGroup.rooms &&
        roomGroup.rooms.forEach((room) => {
          ...
          if (roomName.includes('General') || roomName.includes('Generic')) roomName = 'General Items'
            ...
            room.acm.forEach((acmUid) => {
              ...
              if (acm && acm.sampleType !== 'air') {
                ...
                let row = {
                  ...
                  label:
                    acm.description && acm.material
                      ? acm.description.toLowerCase().includes(acm.material.toLowerCase())
                        ? acm.description
                        : acm.writeItemFirst
                          ? `${acm.description || ''} ${acm.material || ''}`
                ...
                if (acm.idKey === 'i' && acm.sample && acm.sample.result && acm.sample.result.no) {
                  ...
                  if (acm.acmRemoved) {
                    ...
                    if (row.materialRisk !== null && row.priorityRisk !== null)
                      row.totalRisk = getTotalRisk(row.materialRisk, row.priorityRisk)
                ...
                let row = {
                  ...
                  fibreResult: acm.sample.fibreResult || acm.sample.fibreResult === 0 ? acm.sample.fibreResult : 'N/A',
...
export const getAcmRemoved = (acm) => {
  ...
  if (acm.acmRemovalJob) {
    ...
    let str = 'Removed'
    if (acm.acmRemovalJob.removalDate) str = str + ` on ${moment(dateOf(acm.acmRemovalJob.removalDate)).format('D MMMM YYYY')}`
    if (acm.acmRemovalJob.asbestosRemovalist)
      str =
        str +
        ` by ${acm.acmRemovalJob.asbestosRemovalist}${
          acm.acmRemovalJob.asbestosRemovalistLicence ? ` (${acm.acmRemovalJob.asbestosRemovalistLicence})` : ''
        }`
    if (acm.acmRemovalJob.referenceNumber) str = str + ` Clearance Certificate ${acm.acmRemovalJob.referenceNumber} (see APPENDIX I)`
    return str
  } else {
    return `Removed`
  }
...
export const issueDocument =
  ({ template, site, job, registerMap, registerList, airMonitoringRecords, staff }) =>
  (dispatch) => {
    ...
    if (job.versions && Object.keys(job.versions).length > 0) {
      latestIssue = Math.max(...Object.keys(job.versions).map((key) => parseInt(key)))
    }
    ...
    let json = {
      ...
      siteImageUrl: site.siteImageUrl.substring(0, site.siteImageUrl.lastIndexOf('&token')) || null,
      versionHistory
    }
    if (template.includes('AMP')) {
      ...
      json = {
        ...json,
        sitePersonnelNameTertiaryIp402: andList(sitePersonnel.map((e) => nameTertiaryIp402(e))),
        sitePersonnelAsbestosAssessorNumbers: andList(sitePersonnel.map((e) => `${e.asbestosAssessorNumber}`)),

        writerNameTertiaryIp402: andList(authors.writer.map((e) => nameTertiaryIp402(e))),
        checkerNameTertiaryIp402: andList(authors.checker.map((e) => nameTertiaryIp402(e))),
...
export const nameTertiaryIp402 = (e) => {
  return `${e.name}${e.tertiary !== '' && ` (${e.tertiary}${e.ip402 && `, IP402`})`}`
}
...
export const nameFullQuals = (e) => {
  ...
  if (e.asbestosAssessorNumber) quals.push(`Asbestos Assessor No. ${e.asbestosAssessorNumber}`)
  return `${e.name}\n${quals.join(', ')}`
}
...
export const writeAssetOverview = ({ site, job, classDescriptions }) => {
  ...
  if ('aeiou'.includes(site.assetClass[0])) assetDescription = `an ${site.assetClass} Class ${assetDescription}`
  else assetDescription = `a ${site.assetClass} Class ${assetDescription}`
  let bullets = []
  bullets.push(`${site.assetClass}${site.assetNumber} is ${assetDescription} managed by ${job.client}`)
  bullets.push(`It was manufactured by ${site.manufacturedBy}, ${site.countryOfOrigin} in ${site.manufactureYear}`)
...
export const writeRiskOverview = (job, siteAcm) => {
  ...
  if (siteAcm) {
      ...
      if (e.immediateActionRequired) {
        if (e.immediateActionRequired === 'Removal') {
          if (e.removalLicenceRequired === 'Class A') {
            immediateActionsRequired.push(`Removal of the ${label} (Class A removal)`)
          } else if (e.removalLicenceRequired === 'Class B') {
            immediateActionsRequired.push(`Removal of the ${label} (Class B removal)`)
          } else if (e.removalLicenceRequired === 'Unlicensed') {
            immediateActionsRequired.push(`Removal of the ${label} (Unlicensed removal)`)
          ...
          immediateActionsRequired.push(`Arrange further inspection of the ${label}`)
        } else if (e.immediateActionRequired === 'Test') {
          immediateActionsRequired.push(`Arrange further inspection of the ${label}`)
        }
      } else if (e.immediateRisk) immediateActionsRequired.push(`Removal of the ${label}`)
      if (e.shortTermActionText) shortTermActions.push(e.shortTermActionText)
        ...
        if (e.shortTermAction === 'Removal') {
          if (e.removalLicenceRequired === 'Class A') {
            ...
            shortTermActions.push(`Removal of the ${label} (Unlicensed removal)`)
    ...
    // No positive samples and no presumed items
    if (presumedAcm.length === 0 && identifiedAcm.length === 0)
      riskToHealthStr = 'No asbestos-containing materials are present or presumed to be present at this site.'
    if (immediateRisk.length > 0) {
    ...
    if (immediateActionsRequired.length > 0)
      immediateActionsRequiredStr = `<ul>${immediateActionsRequired.map((a) => `<li>${a}</li>`).join('')}</ul>`
    else immediateActionsRequiredStr = 'No immediate actions are required.'
    if (shortTermActions.length > 0) shortTermActionsStr = `<ol>${shortTermActions.map((a) => `<li>${a}</li>`).join('')}</ol>`
  ...
  site.siteVisits.forEach((c) => {
    ...
    if (c.type === 'mgmt')
      dates.push({
        ...
        desc: `An asbestos management survey (${c.referenceNumber}) was conducted by ${andList(
          c.personnel.map(
            (s) =>
              `${s.name}${
                staff[s.uid].tertiary || staff[s.uid].ip402 ? ` (${staff[s.uid].tertiary}${staff[s.uid].ip402 ? `, BOHS IP402` : ''})` : ''
              }`
          )
      ...
      dates.push({
        ...
        desc: `A site visit (${c.referenceNumber}) was conducted by ${andList(
          c.personnel.map(
            (s) =>
              `${s.name}${
                staff[s.uid].tertiary || staff[s.uid].ip402 || staff[s.uid].aanumber
                  ? ` (${staff[s.uid].tertiary}${staff[s.uid].ip402 ? `, BOHS IP402` : ''}${
  ...
  site.asbestosRemovals &&
    Object.values(site.asbestosRemovals).forEach((c) => {
      ...
      dates.push({
        ...
        desc: `An asbestos removal clearance (${c.referenceNumber}) was given by ${andList(
          c.personnel.map((s) =>
            s.name === '3rd Party'
    ...
    roomGroup.rows.forEach((room) => {
      ...
      room.rows.forEach((row) => {
        if (!row.acmRemoved && row.asbestosResult) {
          roomAcm.push(
            `${row.label[0].toUpperCase()}${row.label.slice(1).toLowerCase()} (${
              row.idKey === 'i' ? 'identified' : row.idKey === 's' ? 'strongly presumed' : 'presumed'
            })`.toLowerCase()
          )
        }
      })
      if (roomAcm.length > 0) {
        if (room.label.includes('General') || room.label.includes('Generic')) {
          ...
          acmSummary.push({
            label: room.label,
            summary: roomAcm.map((e) => `${e[0].toUpperCase()}${e.slice(1)}`).join('@~')
          })
      }
    })
  })
...
export const issueTrainAmp =
  ({ site, job, registerMap, registerList, airMonitoringRecords, staff, siteAcm, classDescriptions }) =>
  (dispatch) => {
    console.log(site)
    console.log(job)
    console.log(registerMap)
    ...
    let json = {
      ...
      writer: authors['writer'].length > 0 ? authors['writer'].map((e) => nameFullQuals(e)).join('\n\n') : '',
      checker: authors['checker'].length > 0 ? authors['checker'].map((e) => nameFullQuals(e)).join('\n\n') : '',
      ktp: authors['ktp'].length > 0 ? authors['ktp'].map((e) => nameFullQuals(e)).join('\n\n') : '',
      surveyPersonnel: surveyPersonnel.map((e) => nameFullQualsOneLine(e)).join('\n'),
      ...
      asbestosRemovalRecords: site.asbestosRemovals
        ? Object.values(site.asbestosRemovals).map((c) => ({
            ...
            removalDate: c.removalDate ? moment(dateOf(c.removalDate)).format('DD MMMM YYYY') : '',
            description: c.description,
            clearanceDate: c.clearanceDate ? moment(dateOf(c.clearanceDate)).format('DD MMMM YYYY') : '',
            asbestosAssessorName: c.personnel
              ? c.personnel.map((p) => (p.name === '3rd Party' ? '3rd Party' : `${p.name} (K2 Environmental Ltd)`)).join('\n')
              : '',
            asbestosAssessorLicence: c.personnel
              ? c.personnel.map((p) => (p.name === '3rd Party' ? c.asbestosAssessorLicence || 'N/A' : staff[p.uid].aanumber)).join('\n')
              : '',
            clearanceCertificateRef: c.referenceNumber,
            issueDate: c.issueDate ? moment(dateOf(c.issueDate)).format('DD MMMM YYYY') : ''
          }))
        : '',
      versionHistory,
      airMonitoringRecords,
