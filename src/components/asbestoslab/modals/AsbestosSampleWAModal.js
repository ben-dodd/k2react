...
class AsbestosSampleWAModal extends React.Component {
  ...
  loadProps = () => {
    ...
    Object.values(this.props.samples[this.props.modalProps.activeCoc])
      .filter((s) => s.cocUid === this.props.modalProps.activeCoc)
      .forEach((s) => {
        console.log(s)
  ...
  handleResultClick = (res, sampleNumber) => {
    ...
    if (
      ...
      sample.result &&
      (!this.state.override || !this.state.override[sampleNumber])
    ) {
  ...
  previousSample = () => {
    ...
    Object.values(this.state.samples)
      .reverse()
      .forEach((sample) => {
        if (takeThisSample) {
          this.setState({
  ...
  saveSample = () => {
    ...
    let sample = this.state.samples[this.state.activeSample]
    asbestosSamplesRef.doc(sample.uid).update(sample)
    ...
    if (
      this.props.samples[this.props.modalProps.activeCoc][this.state.activeSample] &&
  ...
  saveMultipleSamples = () => {
    ...
    Object.values(this.state.samples).forEach((sample) => {
      if (
        originalSamples[sample.sampleNumber].result !== sample.result ||
        originalSamples[sample.sampleNumber].weightReceived !== sample.weightReceived
      ) {
        recordAnalysis(
          this.props.analyst,
          sample,
    ...
    return (
      ...
      >
        {this.props.asbestosSampleDisplayAdvanced ? (
          <DialogTitle>
            {sample
              ? `Analysis Details for Sample ${sample.jobNumber}-${sample.sampleNumber} ${writeDescription(sample)}`
              : 'Analysis Details'}
          </DialogTitle>
        ) : (
          <DialogTitle>
            {this.props.cocs[this.props.modalProps.activeCoc]
              ? `Analysis Details for Job ${this.props.cocs[this.props.modalProps.activeCoc].jobNumber}`
              : 'Analysis Details'}
          </DialogTitle>
        )}
        {this.props.asbestosSampleDisplayAdvanced ? (
            ...
            <DialogContent>
              ...
              <Grid container alignItems='flex-start' justify='flex-end'>
                ...
                <Grid item xs={6}>
                  ...
                  <div className={classes.flexRow}>
                    <div className={classes.formInputMedium}>
                      {SamplesTextyBox(
                        ...
                        'weightReceived',
                        'Weight as Received',
                        'Record the weight as received (e.g. entire sample including tape or swab before any conditioning).',
                        false,
                        0,
                        'g',
                        null,
                    ...
                    <div className={classes.formInputMedium}>
                      {SamplesTextyBox(
                        ...
                        'weightSubsample',
                        'Weight of Subsample',
                        'Record the weight of the subsample if the entire sample is not analysed.',
                        false,
                        0,
                        'g',
                        null,
                    ...
                    <div className={classes.formInputMedium}>
                      {SamplesTextyBox(
                        ...
                        'weightDry',
                        'Dry Weight',
                        'Record the weight after drying (~105°).',
                        false,
                        0,
                        'g',
                        null,
                    ...
                    <div className={classes.formInputMedium}>
                      {SamplesTextyBox(
                        ...
                        'weightAshed',
                        'Ashed Weight',
                        'Record the weight after ashing (~400°).',
                        false,
                        0,
                        'g',
                        null,
                  ...
                  {sampleMoisture && <div className={classes.informationBox}>Moisture: {sampleMoisture}%</div>}

                  <div className={classes.subHeading}>Dimensions</div>
                  <div className={classes.flexRow}>
                    <div className={classes.formInputSmall}>
                      {SamplesTextyBox(this, sample, 'dimensionsL', 'Length', null, false, 0, 'mm', null, true)}
                    </div>
                    <span className={classes.timesSymbol}>X</span>
                    <div className={classes.formInputSmall}>
                      {SamplesTextyBox(this, sample, 'dimensionsW', 'Width', null, false, 0, 'mm', null, true)}
                    </div>
                    <span className={classes.timesSymbol}>X</span>
                    <div className={classes.formInputSmall}>
                      {SamplesTextyBox(this, sample, 'dimensionsD', 'Depth/Thickness', null, false, 0, 'mm', null, true)}
                    </div>
                    {sampleDimensions && <span className={classes.informationBox}>{sampleDimensions}</span>}
                  </div>
                  ...
                  {SamplesTextyBox(
                    ...
                    'footnote',
                    null,
                    'Add a footnote to be included in the issued report. This will be displayed as a footnote below the results table.',
                    true,
                    1,
                    null,
                    null
              ...
              <Grid container>
                <Grid item xs={12}>
                  <div className={classes.flexRowLeftAlignEllipsis}>
                    <div className={classes.subHeading}>Result</div>
                  </div>
                  <div className={classes.flexRowRightAlign}>
                    {['ch', 'am', 'cr', 'umf', 'no', 'org', 'smf'].map((res) => {
                      return AsbButton(
                        this.props.classes[`colorsButton${colors[res]}`],
                        this.props.classes[`colorsDiv${colors[res]}`],
                        res,
                  ...
                  <div className={classNames(classes.subHeading, classes.flexRowCenter)}>
                    Layers
                    <IconButton size='small' aria-label='add' className={classes.marginLeftSmall} onClick={this.addLayer}>
                      <AddIcon />
                    </IconButton>
                    <IconButton size='small' aria-label='remove' className={classes.marginLeftSmall} onClick={this.removeLayer}>
                      <RemoveIcon />
                    </IconButton>
                  </div>
              ...
              <Grid container alignItems='flex-start' justify='flex-end'>
                  ...
                  {SamplesRadioSelector(this, sample, 'classification', 'homo', 'Classification', [
                    {
                      ...
                      label: 'Homogenous',
                      tooltip:
                        'Uniform distribution of fibres of any type through the entire sample or in each discernibly discrete layer of the sample (sprayed asbestos, mastic, vermiculite)'
                    },
                    {
                      value: 'homolayers',
                      label: 'Homogenous Layers',
                      tooltip:
                        'Uniform distribution of fibres of any type through each discernibly discrete layer of the sample (asbestos-cement, paper-backed vinyl)'
                    },
                    {
                      value: 'mixedlayers',
                      label: 'Mixed Layers',
                      tooltip: 'Mix of homogenous and non-homogenous layers (asbestos-cement with soil attached)'
                    },
                    {
                      value: 'nonhomo',
                      label: 'Non-homogenous',
                      tooltip:
                        'Small, discrete amounts of asbestos distributed unevenly in a large body of non-asbestos material (e.g. dust, soil)'
                    },
                ...
                <Grid item xs={8}>
                  ...
                  {SamplesTickyBoxGroup(this, sample, 'Analytical Critera', 'analyticalCriteria', [
                    {
                      value: 'dispersion',
                      label: 'Dispersion Staining',
                      tooltip: 'Fibres show positive confirmation of the RIs for both axes for each asbestos type detected.'
                    },
                    { value: 'morphology', label: 'Morphology', tooltip: 'Fibres show appropriate and basic morphological features.' },
                    {
                      value: 'pleochroism',
                      label: 'Pleochroism',
                      tooltip: 'PP: EW-NS blue-grey indicates Crocidolite, other types have no change.'
                    },
        ...
        {this.props.asbestosSampleDisplayAdvanced && sample ? (
          <DialogActions>
            ...
            <Button
              ...
              color='inherit'
              disabled={this.state.samples && Object.values(this.state.samples)[0].uid == sample.uid}
            >
              Previous
            </Button>
            <Button
              ...
              color='secondary'
              disabled={
                this.state.samples && Object.values(this.state.samples)[Object.keys(this.state.samples).length - 1].uid == sample.uid
              }
            >
              Next
            </Button>
      ...
      <div key={num} className={classes.flexRowHoverMed}>
        <div className={classes.flexRow}>
          ...
          <div className={classes.verticalCenter}>
            ...
            {this.state.displayColorPicker[num] ? (
              <div className={classes.colorPickerPopover}>
                <div className={classes.colorPickerCover} onClick={() => this.handleColorClose(num)} />
                <SketchPicker
                  color={sample.layers[`layer${num}`].color}
                  onChangeComplete={(color) => this.setLayerVar('color', num, color.rgb)}
                />
              </div>
            ) : null}
          </div>
        ...
        <div className={classes.flexRowRightAlign}>
          {['ch', 'am', 'cr', 'umf', 'no', 'org', 'smf'].map((res) => {
            return AsbButton(this.props.classes[`colorsButton${colors[res]}`], this.props.classes[`colorsDiv${colors[res]}`], res, (e) =>
              this.toggleLayerRes(res, num)
            )
          })}
        </div>
      </div>
    ...
    return (
      <div key={confirm.date} className={classes.flexRowHoverMed}>
        <div className={classes.flexRow}>
          ...
          <div className={classes.verticalCenter}>{confirm.analyst}</div>
          <div className={classes.verticalCenter}>{moment(confirm.date.toDate()).format('D MMMM YYYY')}</div>
          <div className={classes.verticalCenter}>{confirm.comments}</div>
        </div>
        <div className={classes.flexRowRightAlign}>
          {['ch', 'am', 'cr', 'umf', 'no', 'org', 'smf'].map((res) => {
            return AsbButton(classes[`colorsButton${colors[res]}`], classes[`colorsDiv${colors[res]}`], res, null)
          })}
        </div>
      </div>
    )
...
export default withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(AsbestosSampleWAModal))
