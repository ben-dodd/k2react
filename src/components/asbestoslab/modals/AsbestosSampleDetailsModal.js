  ...
  render() {
    ...
    if (modalType === ASBESTOS_SAMPLE_DETAILS || modalTypeSecondary === ASBESTOS_SAMPLE_DETAILS) {
      ...
      if (sample && sample.waSoilAnalysis)
        soilResult = compareAsbestosResult({ result: collateLayeredResults(sample.waSoilAnalysis) }, sample)
      ...
      if (!modalPropsSecondary || modalPropsSecondary.noNext === undefined)
        samples = Object.values(this.props.samples[job.uid]).filter((s) => s.cocUid === job.uid)
        ...
        <div>
          {job && sample && (modalType === ASBESTOS_SAMPLE_DETAILS || modalTypeSecondary === ASBESTOS_SAMPLE_DETAILS) && (
            ...
            >
              ...
              {(modalType === ASBESTOS_SAMPLE_DETAILS || modalTypeSecondary === ASBESTOS_SAMPLE_DETAILS) && (
                <DialogContent>
                  <Grid container alignItems='flex-start' justify='flex-end'>
                    <Grid item xs={6}>
                      ...
                      <div className={classes.informationBox}>
                        ...
                        >
                          {['ch', 'am', 'cr', 'umf', 'no', 'org', 'smf'].map((res) => {
                            if (sample.result && sample.result[res] === true)
                        ...
                        {sample.analysisRecordedBy &&
                          ...
                          SampleTextyLine('Analysis Recorded By', sample.analysisRecordedBy.name)}
                        ...
                        {sample.layers && Object.keys(collateLayeredResults(sample.layers)).length > 0 && (
                          <div
                            style={{
                              display: 'flex',
                              flexDirection: 'row'
                          ...
                          >
                            <div style={{ width: '60%' }}>
                              {SampleTextyLine('Cumulative Layer Results', writeShorthandResult(collateLayeredResults(sample.layers)))}
                            </div>
                            <div style={{ width: '40%' }}>
                              {layersResult === 'yes' || layersResult === 'differentNonAsbestos'
                                ? good
                                : layersResult === 'no'
                                  ? bad
                                  : layersResult === 'none'
                        ...
                        {sample.waSoilAnalysis && (
                          <div style={{ display: 'flex', flexDirection: 'row' }}>
                            <div style={{ width: '60%' }}>
                              {SampleTextyLine('WA Standard Results', writeShorthandResult(collateLayeredResults(sample.waSoilAnalysis)))}
                            </div>
                            <div style={{ width: '40%' }}>
                              {soilResult === 'yes' || soilResult === 'differentNonAsbestos'
                                ? good
                                : soilResult === 'no'
                                  ? bad
                                  : soilResult === 'none'
                              ...
                              return (
                                ...
                                >
                                  <div style={{ width: '60%' }}>
                                    {SampleTextyLine(
                                      `Sample Check ${key}`,
                                      `${writeShorthandResult(sample.confirm[key].result)} (${sample.confirm[key].analyst})`
                                    )}
                                  </div>
                                  <div style={{ width: '40%' }}>
                                    {check === 'yes' || check === 'differentNonAsbestos'
                                      ? good
                                      : check === 'no'
                                        ? bad
                                        : check === 'none'
                      ...
                      <div className={classes.informationBox}>
                        ...
                        {sample.classification !== undefined &&
                          SampleTextyLine('Homogenous?', sample.classification === 'homo' ? 'Yes' : 'No')}
                        {sample.asbestosEvident !== undefined &&
                          SampleTextyLine('Asbestos Evident?', sample.asbestosEvident ? 'Yes' : 'No')}
                    ...
                    <Grid item xs={6}>
                      <div className={classes.informationBox}>
                        ...
                        {SampleTextyLine(
                          'Sampling Personnel',
                          sample.sampledBy && sample.sampledBy.length > 0 ? sample.sampledBy.map((e) => e.name).join(', ') : 'Not specified'
                        )}
                        {SampleTextyLine(
                          'Sampling Date(s)',
                          sample.sampleDate ? moment(dateOf(sample.sampleDate)).format('dddd, D MMMM YYYY') : 'Not specified'
                        )}
                        <div
                          style={{
                            display: 'flex',
                        ...
                        {sampleMoisture && <div>{SampleTextyDisplay('Moisture Content', `${sampleMoisture}%`)}</div>}
                        ...
                        {sample.layers &&
                          ...
                          SampleTextyDisplay(
                            'Layers',
                            [...Array(sample.layerNum ? sample.layerNum : 5).keys()]
                              .filter(
                                (num) =>
                                  sample.layers[`layer${num + 1}`] &&
                                  ((sample.layers[`layer${num + 1}`].description !== '' &&
                                    sample.layers[`layer${num + 1}`].description !== undefined) ||
                                    (sample.layers[`layer${num + 1}`].result &&
                                      Object.keys(sample.layers[`layer${num + 1}`].result).length > 0))
                              )
                      ...
                      <div className={classes.informationBox}>
                        ...
                        {sample.receivedDate
                          ? sample.verified
                            ? SampleTextyLine('Turnaround Time (Total)', milliToDHM(timeTotal, true, false))
                        ...
                        {sample.receivedDate
                          ? sample.verified
                            ? SampleTextyLine('Turnaround Time (Business Hours Only)', milliToDHM(timeTotalBusiness, true, true))
                        ...
                        {sample.receivedDate
                          ? sample.analysisDate
                            ? SampleTextyLine('Lab Time (Total)', milliToDHM(timeLab, true, false))
                            : SampleTextyLine('Time In Lab (Total)', milliToDHM(timeLab, true, false))
                          : SampleTextyLine('Lab Time (Total)', 'Not yet received by lab')}
                        {sample.receivedDate
                          ? sample.analysisDate
                            ? SampleTextyLine('Lab Time (Business Hours Only)', milliToDHM(timeLabBusiness, true, true))
                        ...
                        {sample.analysisDate
                          ? sample.verified
                            ? SampleTextyLine('Admin Time (Total)', milliToDHM(timeAdmin, true, false))
                        ...
                        {sample.analysisDate
                          ? sample.verified
                            ? SampleTextyLine('Admin Time (Business Hours Only)', milliToDHM(timeAdminBusiness, true, true))
                      ...
                      <div className={classes.informationBox}>
                        ...
                        {SampleTextyLine(
                          'Created',
                          sample.createdDate
                            ? `${moment(sample.createdDate.toDate()).format('h:mma, dddd, D MMMM YYYY')} by ${
                                sample.createdBy ? sample.createdBy.name : 'an unknown person'
                              }`
                            : 'No creation date'
                        )}
                        {SampleTextyLine(
                          'Received by Lab',
                          sample.receivedDate
                            ? `${moment(sample.receivedDate.toDate()).format('h:mma, dddd, D MMMM YYYY')} by ${
                                sample.receivedBy ? sample.receivedBy.name : 'an unknown person'
                              }`
                        ...
                        {SampleTextyLine(
                          ...
                          sample.analysisStartDate
                            ? `${moment(sample.analysisStartDate.toDate()).format('h:mma, dddd, D MMMM YYYY')} by ${
                                sample.analysisStartedBy ? sample.analysisStartedBy.name : 'an unknown person'
                              }`
                        ...
                        {SampleTextyLine(
                          'Result Logged',
                          sample.analysisDate
                            ? `${moment(sample.analysisDate.toDate()).format('h:mma, dddd, D MMMM YYYY')} by ${
                                sample.analysisRecordedBy ? sample.analysisRecordedBy.name : 'an unknown person'
                              }`
                        ...
                        {SampleTextyLine(
                          'Result Verified',
                          sample.verifyDate
                            ? `${moment(sample.verifyDate.toDate()).format('h:mma, dddd, D MMMM YYYY')} by ${
                                sample.verifiedBy ? sample.verifiedBy.name : 'an unknown person'
                              }`
                  ...
                  {job.waAnalysis && (
                    <div className={classes.informationBox}>
                      ...
                      <AsbestosSampleWASummary
                        ...
                        acmInSoilLimit={job.acmInSoilLimit ? parseFloat(job.acmInSoilLimit) : 0.01}
                      />
                    </div>
                  )}
                </DialogContent>
              ...
              <DialogActions>
                {(!modalPropsSecondary || modalPropsSecondary.noNext === undefined) && (
                  <span>
                    <Button onClick={() => this.previousSample(samples)} color='inherit' disabled={samples[0].uid == sample.uid}>
                      Previous
                    </Button>
                    <Button
      ...
      >
        <div
          style={{
            ...
            }, ${layer.color ? layer.color.b : null}, ${layer.color ? layer.color.a : null})`,
            marginRight: 10,
            color: '#fff',
        ...
        <div style={{ width: '30%' }}>{layer.concentration && `Approx. ${layer.concentration}% asbestos content`}</div>
      </div>
    )
  }
}

export default withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(AsbestosSampleDetailsModal))
