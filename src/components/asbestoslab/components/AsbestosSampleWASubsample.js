...
class AsbestosSampleWASubsample extends React.Component {
  state = {
    form: this.props.subsample.formDescription ? this.props.subsample.formDescription : ''
  }
        ...
        <Grid item xs={12} lg={6} className={classes.flexRow}>
              ...
              value={
                subsample.concentration
                  ? {
                      ...
                      label: this.props.asbestosInSoilConcentrations.filter((e) => e.value === subsample.concentration)[0].label
                    }
                  : { value: '', label: '' }
              }
          ...
          <div className={classes.columnMedSmall}>
            <Select
              value={
                subsample.form
                  ? { value: subsample.form, label: this.props.asbestosInSoilForms.filter((e) => e.value === subsample.form)[0].label }
                  : { value: '', label: '' }
              }
              options={
                subsample.fraction === 'gt7'
          ...
          <div className={classes.columnMedSmall}>
            <div className={classNames(classes.informationBoxRounded, classes.bold)}>
              {subsample.concentration && subsample.weight
                ? subsample.tareWeight
                  ? (
                      ((parseFloat(subsample.weight) - parseFloat(subsample.tareWeight)) * parseFloat(subsample.concentration)) /
                      100
                    ).toFixed(5) + 'g'
...
export default withStyles(styles)(connect(mapStateToProps, null)(AsbestosSampleWASubsample))
