...
class JobMap extends React.Component {
  ...
  applyFilters = (m) => {
    ...
    if (!m.isJob) state = 'Lead'
      ...
      "needs writingneeds finishingbeing writtenready to be checkedbeing checkedbeen checked, needs fixingready for ktpbeing ktp'dktped, actions needed".includes(
        state
      )
    )
      state = 'Report'
    ...
    // Category
    if (this.props.filter.filterOnCategory && this.props.filter.filterCategory !== category) return false
    ...
    if (this.props.filter.filterUpdatedInTheLast && getDaysSinceDate(m.lastActionDate) < this.props.filter.updatedInTheLast) return false

    // Actions overdue by
    if (this.props.filter.filterActionsOverdueBy && (m.isJob || getNextActionOverdueBy(m.activities) < this.props.filter.actionsOverdueBy))
      return false
  ...
  render() {
    ...
    const jobListModal = (
      <Dialog maxWidth='lg' fullWidth={true} open={this.state.modal !== null} onClose={() => this.setState({ modal: null })}>
        <DialogTitle>{this.state.modal === 'nolocation' ? 'Jobs or Leads with No Location Data' : 'List View'}</DialogTitle>
        <DialogContent>
          {this.props.jobList &&
            Object.values(this.props.jobList)
              .filter((m) => this.applyModalFilters(m, false))
              .sort((a, b) => {
    ...
    return (
      <div className={classes.marginBottomSmall}>
        ...
        <ExpansionPanel>
          <ExpansionPanelSummary expandIcon={<ExpandMore />}>FILTER VIEW</ExpansionPanelSummary>
          <ExpansionPanelDetails>
            <div>
              <Grid container spacing={1}>
                <Grid item>
                  <Button variant='outlined' color='default' onClick={this.resetFilter}>
                    Reset Filter
                  </Button>
                </Grid>
                {['Leads', 'Jobs'].map((chip) => {
                  return (
                    <Grid item key={chip}>
                      <Button
                        variant='outlined'
                        color={this.props.filter.filterJobLead === chip ? 'secondary' : 'default'}
              ...
              <Grid container spacing={1} className={classes.marginBottomSmall}>
                {['Lead', 'On Hold', 'Needs Booking', 'Planned', 'Site Work', 'Lab', 'Report', 'Admin', 'Completed'].map((chip) => {
                  return (
                    <Grid item key={chip}>
                      <Button
                        variant='outlined'
                        color={this.props.filter.filterState === chip ? 'secondary' : 'default'}
              ...
              <Grid container spacing={1} className={classes.marginBottomSmall}>
                {['Asbestos', 'Meth', 'Stack', 'Noise', 'Bio', 'Workplace', 'Other'].map((chip) => {
                  return (
                    <Grid item key={chip}>
                      <Button
                        className={classes[getJobColor(chip)]}
                        variant='outlined'
                        color={this.props.filter.filterCategory === chip ? 'secondary' : 'default'}
